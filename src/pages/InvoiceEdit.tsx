...
export default function InvoiceEdit() {
  ...
  useEffect(() => {
    if (invoice) {
      ...
      setLines(invoice.items);
      setInvoiceDate(invoice.date);
      setDueDate(invoice.dueDate);
      setIrnNo(invoice.irnNo || '');
      setAckNo(invoice.ackNo || '');
      setAckDate(invoice.ackDate || '');
      setOrderNo(invoice.orderNo || '');
      setRemark(invoice.remark || '');
      setFreightCharges(invoice.freightCharges || 0);
      setStatus(invoice.status === 'Paid' ? 'Active' : (invoice.status as any));
      setUseSeparateDelivery(!!invoice.deliveryAddress?.address && invoice.deliveryAddress.address !== invoice.customerAddress);
    ...
    try {
      ...
      if (id) {
        await supabase.from('invoices').update({
          ...invoicePayload,
          data: invoicePayload,
          invoice_no: invoicePayload.invoiceNumber,
          customer_id: invoicePayload.customerId,
          customer_name: invoicePayload.customerName,
          customer_gstin: invoicePayload.customerGstin,
          customer_pan: invoicePayload.customerPan,
          customer_aadhar: invoicePayload.customerAadhar,
          customer_phone: invoicePayload.customerPhone,
          customer_email: invoicePayload.customerEmail,
          customer_address: invoicePayload.customerAddress,
          customer_city: invoicePayload.customerCity,
          customer_state: invoicePayload.customerState,
          customer_state_code: invoicePayload.customerStateCode,
          delivery_address: invoicePayload.deliveryAddress.address,
          delivery_city: invoicePayload.deliveryAddress.city,
          delivery_state: invoicePayload.deliveryAddress.state,
          delivery_state_code: invoicePayload.deliveryAddress.stateCode,
          delivery_pincode: invoicePayload.deliveryAddress.pincode,
          date: invoicePayload.date.split('T')[0],
          due_date: invoicePayload.dueDate.split('T')[0],
          order_no: invoicePayload.orderNo,
          remark: invoicePayload.remark,
          subtotal_mrp: invoicePayload.subtotalMrp,
          total_discount_amount: invoicePayload.totalDiscountAmount,
          subtotal_basic: invoicePayload.subtotalBasic,
          total_cgst: invoicePayload.totalCgst,
          total_sgst: invoicePayload.totalSgst,
          total_igst: invoicePayload.totalIgst,
          freight_charges: invoicePayload.freightCharges,
          freight_gst: invoicePayload.freightGst,
          round_off: invoicePayload.roundOff,
          grand_total: invoicePayload.grandTotal,
          items: invoicePayload.items
        }).eq('id', id);

        // Update items table (simplest approach: delete and re-insert)
  ...
  if (isInvoiceLoading) return <div className="p-8 text-center"><Loader className="animate-spin mx-auto" /></div>;

  return (
    <div className="space-y-6 pb-20">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-2xl font-black text-slate-900 dark:text-slate-50 uppercase tracking-tight">Edit Tax Invoice</h2>
          <p className="text-sm text-slate-500 font-medium mt-1">Modifying invoice: <span className="font-bold text-indigo-600">{invoice?.invoiceNumber}</span></p>
        </div>
        <div className="flex gap-2">
          <button 
            disabled={isSaving || lines.length === 0}
            onClick={saveInvoice}
            className="inline-flex items-center gap-2 rounded-3xl bg-slate-900 px-8 py-3 text-sm font-bold text-white transition hover:bg-slate-800 disabled:opacity-50 shadow-lg active:scale-95"
          >
            {isSaving ? <Loader className="w-4 h-4 animate-spin" /> : <CreditCard className="w-4 h-4" />}
            {isSaving ? 'Updating...' : 'Update & Save Invoice'}
          </button>
        </div>
      </header>
        ...
        <aside className="space-y-6">
          <div className="glass-card rounded-3xl border border-slate-200 p-6 shadow-xl dark:border-slate-800 sticky top-6 bg-slate-900 text-white">
            <h3 className="text-lg font-black uppercase tracking-tight flex items-center gap-2 border-b border-white/10 pb-4 mb-6">
              <Calculator className="w-5 h-5 text-indigo-400" /> Billing Totals
            </h3>
            
            <div className="space-y-4">
              <div className="space-y-1 mb-4">
                <label className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Freight Charges (₹)</label>
                <input 
                  type="number" 
                  ...
                  placeholder="0.00"
                />
                <p className="text-[9px] text-indigo-400 font-bold italic">+ 18% GST will be applied automatically</p>
              </div>
              ...
              <div className="flex justify-between text-sm font-black text-indigo-400">
              ...
              <div className="pt-6 mt-6 border-t border-white/20">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Bill Amount Payable</p>
                ...
                <div className="mt-4 p-3 bg-white/5 rounded-xl border border-white/5">
                  <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-1">Amount in words</p>
            ...
            <div className="mt-8 space-y-3">
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Invoice State Status</label>
              <select value={status} onChange={e => setStatus(e.target.value as any)} className="w-full rounded-2xl bg-white/10 border border-white/20 px-4 py-3 text-sm font-bold outline-none focus:bg-white/20 transition cursor-pointer">
                <option value="Active" className="bg-slate-900">Final (Active)</option>
                <option value="Draft" className="bg-slate-900">Draft (Temporary)</option>
              </select>

              <button 
                disabled={isSaving || lines.length === 0}
                onClick={saveInvoice}
                className="w-full rounded-3xl bg-indigo-600 px-5 py-4 text-sm font-black text-white transition hover:bg-indigo-500 disabled:opacity-50 shadow-2xl active:scale-95 flex items-center justify-center gap-3 mt-4"
              >
                {isSaving ? <Loader className="w-5 h-5 animate-spin" /> : <span>💳</span>}
                {isSaving ? 'UPDATING...' : 'FINALIZE & SAVE'}
              </button>
            </div>
          </div>
